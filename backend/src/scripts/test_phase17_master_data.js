...
async function runTests() {
  ...
  try {
    await floorService.createFloor({
      hostel_id: createdHostel.id,
      floor_name: 'Audit First Floor Dup',
      floor_number: 1
    }, mockSuperAdmin);
  ...
  // Occupied Bed Modification Safety
  try {
    // Bed #1 is OCCUPIED in mock database
    await masterService.validateBedDeactivation(1, 'INACTIVE');
    assert(false, 'Occupied bed deactivation should be blocked');
  } catch (err) {
    assert(err.status === 400 && /occupied/i.test(err.message), 'Occupied bed deactivation blocked with safety error');
  }
