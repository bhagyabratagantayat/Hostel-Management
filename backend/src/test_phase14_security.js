...
async function runSecurityTests() {
  ...
  try {
    await userService.updateUserStatus(4, 'INACTIVE', superAdminActor, { ip_address: '127.0.0.1' });
    const res = await authService.validateUser('student2', 'password123', { ip_address: '127.0.0.1' });
    assert.deepStrictEqual(res, { error: 'ACCOUNT_INACTIVE' }, 'Inactive account login must return ACCOUNT_INACTIVE');
    await userService.updateUserStatus(4, 'ACTIVE', superAdminActor, { ip_address: '127.0.0.1' });
  ...
  try {
    const res = await authService.changePassword(newUserId, 'TempPassword123!', 'NewStrongPass123!', { ip_address: '127.0.0.1' });
  ...
  try {
    const resetRes = await userService.adminResetPassword(newUserId, 'ResetAdminPass123!', superAdminActor, { ip_address: '127.0.0.1' });
  ...
  try {
    const updateH = await userService.updateSuperintendentHostels(newUserId, [2, 3], superAdminActor, { ip_address: '127.0.0.1' });
    assert.deepStrictEqual(updateH.assigned_hostel_ids, [2, 3]);

    // Change role to STUDENT -> should clear hostel assignments
    await userService.updateUserRole(newUserId, 'STUDENT', superAdminActor, { ip_address: '127.0.0.1' });

    let errCaught = false;
    try {
      await userService.updateSuperintendentHostels(newUserId, [1], superAdminActor, { ip_address: '127.0.0.1' });
    } catch (e) {
      errCaught = true;
  ...
  try {
    const profRes = await userService.updateSelfProfile(3, { email: 'student_updated@hostel.com', phone_number: '9998887776', role: 'SUPER_ADMIN' }, { ip_address: '127.0.0.1' });
    assert.strictEqual(profRes.success, true);

    const checkProf = await userService.getUserById(3);
